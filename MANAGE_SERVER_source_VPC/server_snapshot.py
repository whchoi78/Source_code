...
class ServerSnapshot:
	...
	def create_snapshot(self, storage_name, storage_id, max_count):
		...
		req_path = '/vserver/v2/createBlockStorageSnapshotInstance?blockStorageInstanceNo=' + storage_id \
	...
	def get_storage_snapshot_name(self, storage_id, snapshot_name):
		req_path = '/vserver/v2/getBlockStorageSnapshotInstanceList?originalBlockStorageInstanceNoList.1=' + storage_id + '&responseFormatType=json'. \
			format(storage_id)
	...
	def req_delete_storage_snapshot(self, snapshot_id):		
		req_path = '/vserver/v2/deleteBlockStorageSnapshotInstances?blockStorageSnapshotInstanceNoList.1=' + snapshot_id + '&responseFormatType=json'. \
			format(snapshot_id)
		self.sender(req_path)
	...
	def get_server_storage_list(self, server_name, sever_id):
		req_path = '/vserver/v2/getBlockStorageInstanceList?serverInstanceNo=' + sever_id + '&responseFormatType=json'. \
			format(sever_id)
		res = self.sender(req_path)
	...
	def get_storage(self, storage_id):
		req_path = '/vserver/v2/getBlockStorageInstanceList?blockStorageInstanceNoList.1=' + storage_id + '&responseFormatType=json'. \
			format(storage_id)
		res = self.sender(req_path)
