...
class LBControll:
	...
	def info_lb(self, lb_id):
		req_path = '/vloadbalancer/v2/getLoadBalancerInstanceList?loadBalancerInstanceNoList.1=' + lb_id +  '&responseFormatType=json'. \
			format(lb_id)
	...
	def valid_server_from_lb(self, lb_id, server_name):
		req_path = '/vloadbalancer/v2/getLoadBalancedServerInstanceList?loadBalancerInstanceNo=' + lb_id +  '&responseFormatType=json'. \
			format(lb_id)
		res = self.sender(req_path)
