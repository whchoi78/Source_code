...
class ValidServer:
	def valid_server_status(self, server_name, status_code):
		req_path = '/vserver/v2/getServerInstanceList?searchFilterName=serverName&searchFilterValue=' + server_name + '&serverInstanceStatusCode=' + status_code + '&responseFormatType=json'. \
			format(server_name)
	...
	def valid_server_normal_status(self, server_name):
		req_path = '/vserver/v2/getServerInstanceList?searchFilterName=serverName&searchFilterValue=' + server_name + '&responseFormatType=json'. \
			format(server_name)
		res = self.sender(req_path)		
