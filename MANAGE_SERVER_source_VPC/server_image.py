...
class ServerImage:
	...
	def force_create_server_image(self, server_name, max_count):
		...
		if not (server_valid.valid_server(server_name)):			
			return
		server_ids = server_valid.valid_server_status(server_name, "RUN")
	...
	def get_server_image_name(self, server_name, image_name):		
		req_path = '/vserver/v2/getMemberServerImageList?responseFormatType=json'. \
			format(server_name)
	...
	def req_create_server_image(self, server_ids, server_name, image_name):		
		req_path = '/vserver/v2/createMemberServerImage?serverInstanceNo=' + server_ids \
	...
	def req_delete_server_image(self, server_name):		
		req_path = '/vserver/v2/deleteMemberServerImages?memberServerImageNoList.1=' + server_name + '&responseFormatType=json'. \
			format(server_name)
		self.sender(req_path)
